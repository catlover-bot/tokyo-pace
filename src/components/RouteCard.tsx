...
function Badges({ model, selected }: { model: RouteComparisonViewModel; selected: boolean }) {
  const badges = [model.isRecommended && "TOKYO PACE推奨", model.isShortest && "最短", model.isFastest && "最短時間", model.route.profile === "step_avoiding" && "階段回避要求", model.route.profile === "wheelchair_profile" && "車いすプロファイル候補", model.meetsPreferences ? "必須条件達成" : "必須条件未達", model.isFallback ? "固定デモ" : "OpenRouteService", "公式データ使用", selected && "地図で選択中"].filter(Boolean) as string[];
  return <div className="badges" aria-label="経路の特徴">{[...new Set(badges)].map((badge) => <span key={badge}>{badge}</span>)}</div>;
}

export function RouteCard({ model, selected, onSelect }: { model: RouteComparisonViewModel; selected: boolean; onSelect(routeId: string): void }) {
  const route = model.route;
  return <article className={`route-card profile-${model.visualPattern}${selected ? " selected" : ""}${model.isRecommended ? " recommended-route" : ""}`} aria-labelledby={`route-${model.routeId}-title`} aria-current={selected ? "true" : undefined} data-profile={route.profile ?? "demo"}>
    <div className="card-rank"><strong>推奨順位 {model.rank}位</strong><span>{model.profileLabel}</span></div>
    <div className="card-top"><div><h3 id={`route-${model.routeId}-title`}>{model.routeName}</h3><Badges model={model} selected={selected} /></div></div>
    ...
    <div className={`continuity-status ${model.continuityFeasible ? "feasible" : "exceeded"}`}><strong>歩き続ける時間：{model.continuityFeasible ? "設定内" : "設定超過"}</strong><span>{model.continuityFeasible ? "各区間の推定歩行時間が設定上限内です。" : `設定${route.continuousWalkingLimitMinutes}分に対して最大${model.maxContinuousWalkingMinutes}分です。`}</span></div>
    <dl className="metrics primary-metrics"><div><dt>最大連続歩行</dt><dd>{model.maxContinuousWalkingMinutes}分</dd></div><div><dt>最長休憩空白</dt><dd>推定{model.longestRestGapMeters}m</dd></div><div><dt>最長公衆トイレ空白</dt><dd>推定{model.longestPublicToiletGapMeters}m</dd></div><div><dt>最長給水空白</dt><dd>推定{model.longestDrinkingWaterGapMeters}m</dd></div><div><dt>確認できた休憩場所のつながり</dt><dd>{model.strictRestNetworkFeasible ? "設定内" : "超過区間あり"}</dd></div><div><dt>利用者条件</dt><dd>{model.meetsPreferences ? "満たす" : "未達あり"}</dd></div></dl>
    ...
    <div className="card-lists"><section><h4>利点</h4><ul>{model.advantages.length ? model.advantages.map((item) => <li key={item.code}>{item.text}</li>) : <li>ほかの候補と指標を比較してください</li>}</ul></section><section><h4>注意点・条件未達</h4>{model.constraintViolations.length === 0 && model.tradeoffs.length === 0 ? <p>入力した必須条件の未達はありません。</p> : <ul>{[...model.constraintViolations, ...model.tradeoffs].map((item, index) => <li key={`${item.code}-${index}`}>{item.text}</li>)}</ul>}</section></div>
