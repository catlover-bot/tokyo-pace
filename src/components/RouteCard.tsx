...
export function RouteCard({ model, selected, expanded, topRouteMeetsPreferences, onSelect, onToggleDetails }: { model: RouteComparisonViewModel; selected: boolean; expanded: boolean; topRouteMeetsPreferences: boolean; onSelect(routeId: string): void; onToggleDetails(routeId: string): void }) {
  ...
  return <article className={`route-card profile-${model.visualPattern}${selected ? " selected" : ""}${model.isRecommended ? " recommended-route" : ""}`} aria-labelledby={titleId} aria-current={selected ? "true" : undefined} data-profile={route.profile ?? "demo"}>
    ...
    <div id={detailsId} className="route-card-details" role="region" aria-labelledby={detailsButtonId} hidden={!expanded}>
      ...
      {route.elevation?.status && route.elevation.status !== "unavailable" ? <section className="explanation-block" aria-label="標高から推定した坂道情報"><h4>坂道の詳細（標高からの推定）</h4><dl className="metrics"><div><dt>累積上昇量</dt><dd>推定{Math.round(route.elevation.totalAscentMeters ?? 0)}m</dd></div><div><dt>累積下降量</dt><dd>推定{Math.round(route.elevation.totalDescentMeters ?? 0)}m</dd></div><div><dt>推定最大勾配</dt><dd>{(route.elevation.maximumEstimatedGradePercent ?? 0).toFixed(1)}%</dd></div><div><dt>5%以上の上り</dt><dd>推定{Math.round(route.elevation.uphillDistanceAboveThresholdMeters ?? 0)}m</dd></div><div><dt>8%以上の急な上り候補</dt><dd>推定{Math.round(route.elevation.steepUphillDistanceMeters ?? 0)}m</dd></div><div><dt>最長の連続上り</dt><dd>推定{Math.round(route.elevation.longestContinuousUphillMeters ?? 0)}m</dd></div></dl><p className="metric-help">国土地理院の標高タイルからTOKYO PACEが算出した推定値です。現地測定や通行可能性の保証ではありません。標高取得率：{Math.round(route.elevation.completenessRatio * 100)}%</p></section>
      : <p className="warning-text">坂道情報を取得できませんでした。0mとしては扱わず、ほかの指標で比較しています。</p>}
