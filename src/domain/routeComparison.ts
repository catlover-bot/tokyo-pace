...
import { classifyRestContinuity, type RestContinuityPresentation } from "./restContinuityPresentation";
import { deriveRoutePreferenceSummary, type RoutePreferenceSummary } from "./routeRecommendationLabel";
...
export type RouteComparisonViewModel = {
  ...
  restContinuity: RestContinuityPresentation;
...
export type RouteComparisonResult = RoutePreferenceSummary & {
  baselineRouteId: string | null;
  recommendedRouteId: string | null;
  routes: RouteComparisonViewModel[];
};

const durationSeconds = (route: EvaluatedRoute) => route.durationSeconds ?? route.durationMinutes * 60;
const isStrictRestNetworkFeasible = (presentation: RestContinuityPresentation) => presentation.classification === "STRICT_REST_POINTS_MAKE_ROUTE_FEASIBLE";
...
function violations(route: EvaluatedRoute, preferences: RoutePreferences, restContinuity: RestContinuityPresentation): ExplanationItem[] {
  ...
  if (!restContinuity.continuityWithinLimit) result.push({ code: "STRICT_REST_NETWORK_NOT_FEASIBLE", text: restContinuity.description });
  return result;
}

function recommendationReasons(route: EvaluatedRoute, routes: EvaluatedRoute[], baseline: EvaluatedRoute, preferences: RoutePreferences, maximum: number, restContinuity: RestContinuityPresentation): ExplanationItem[] {
  const reasons: ExplanationItem[] = [];
  const baselineLabel = baseline.profile === "standard" || baseline.id === "standard" ? "標準候補" : "比較基準候補";
  ...
  if (preferences.avoidSteepSlopes && route.id !== baseline.id && route.elevation?.totalAscentMeters != null && baseline.elevation?.totalAscentMeters != null && route.elevation.totalAscentMeters < baseline.elevation.totalAscentMeters) reasons.push({ code: "LOWER_ESTIMATED_ASCENT", text: `${baselineLabel}より推定累積上昇量が${roundComparisonValue(baseline.elevation.totalAscentMeters - route.elevation.totalAscentMeters, 1)}m少ない候補です` });
  if (preferences.avoidSteepSlopes && route.id !== baseline.id && route.elevation?.steepUphillDistanceMeters != null && baseline.elevation?.steepUphillDistanceMeters != null && route.elevation.steepUphillDistanceMeters < baseline.elevation.steepUphillDistanceMeters) reasons.push({ code: "LOWER_STEEP_UPHILL_DISTANCE", text: `推定8%以上の急な上り候補が${baselineLabel}より${roundComparisonValue(baseline.elevation.steepUphillDistanceMeters - route.elevation.steepUphillDistanceMeters)}m短い候補です` });
  ...
  if (isStrictRestNetworkFeasible(restContinuity)) reasons.push({ code: "STRICT_REST_NETWORK_FEASIBLE", text: restContinuity.description });
  if (restContinuity.classification === "WHOLE_ROUTE_WITHIN_LIMIT_WITHOUT_STRICT_REST") reasons.push({ code: "WHOLE_ROUTE_WITHIN_LIMIT", text: restContinuity.description });
  return reasons.slice(0, maximum);
}

function advantages(route: EvaluatedRoute, routes: EvaluatedRoute[], preferences: RoutePreferences, restContinuity: RestContinuityPresentation): ExplanationItem[] {
  ...
  if (route.meetsPreferences) items.push({ code: "MEETS_ALL_REQUIRED_PREFERENCES", text: "設定した希望条件をすべて満たす" });
  ...
  if (isStrictRestNetworkFeasible(restContinuity)) items.push({ code: "STRICT_REST_NETWORK_FEASIBLE", text: restContinuity.description });
  if (restContinuity.classification === "WHOLE_ROUTE_WITHIN_LIMIT_WITHOUT_STRICT_REST") items.push({ code: "WHOLE_ROUTE_WITHIN_LIMIT", text: restContinuity.description });
  return items;
}
...
export function buildRouteComparisonViewModels(evaluatedRoutes: readonly EvaluatedRoute[], preferences: RoutePreferences, maximumReasons = 4): RouteComparisonResult {
  if (evaluatedRoutes.length === 0) return { baselineRouteId: null, recommendedRouteId: null, routes: [], ...deriveRoutePreferenceSummary([]) };
  ...
  const models = routes.map((route, index): RouteComparisonViewModel => {
    const restContinuity = classifyRestContinuity(route);
    ...
    const constraintViolations = violations(route, preferences, restContinuity);
    ...
    return {
      ...
      strictRestCandidateCount: restContinuity.strictIntermediateRestPointCount, possibleRestCandidateCount: route.referencePossibleCandidateCount, drinkingStationCount: route.drinkingStationCount,
      continuityFeasible: route.continuityFeasible, strictRestNetworkFeasible: isStrictRestNetworkFeasible(restContinuity), meetsPreferences: route.meetsPreferences, restContinuity,
      recommendationReasons: recommendationReasons(route, routes, baseline, preferences, maximumReasons, restContinuity), advantages: advantages(route, routes, preferences, restContinuity), tradeoffs, constraintViolations, safetyWarnings: safetyWarnings(route),
  ...
  return { baselineRouteId: baseline.id, recommendedRouteId: routes[0].id, routes: models, ...deriveRoutePreferenceSummary(routes) };
}
