...
- 提供者：国土地理院
- データセット：標高タイル（基盤地図情報数値標高モデル）
- 公式仕様：https://maps.gsi.go.jp/development/demtile.html
- 公式一覧：https://maps.gsi.go.jp/development/ichiran.html#dem
- 取得：Workerからzoom 15テキストタイルをDEM5A → DEM5B → DEM10Bの順に取得し、タイル単位で重複排除
- 座標系：Web Mercatorの地理院タイル座標。入力経緯度はWGS84相当のroute geometry
- 規約：国土地理院コンテンツ利用規約。表示は「国土地理院の標高タイルを加工して作成」
- 派生：25m標本、推定grade、slope class、累積上昇・下降、5%/8%上り距離、最長連続上り
- 制約：テキスト標高タイルは2024年10月以降更新停止。DEMの解像度、平滑化、建築物・道路構造、入口、階段、工事、現況は表さず、現地測定ではない
- 欠損：`e`、通信失敗、malformed tile、取得率80%未満はunknown。0へ補完しない
