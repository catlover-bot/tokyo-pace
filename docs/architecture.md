...
`worker/gsiElevation.ts` はORS成功後にだけ動作する任意のenrichmentです。3経路を25m間隔で再標本化し、zoom 15のGSIテキスト標高タイルを座標量子化・タイル重複排除して一括取得します。DEM5A、DEM5B、DEM10Bの順に欠損をフォールバックし、同じタイルを経路間で再利用します。失敗はroute API失敗へ昇格させません。

`src/domain/elevation.ts` は raw sample → duplicate除去/resample → 3点中央値 → 25m区間grade → slope class → route metrics の純粋関数です。取得率80%未満はunavailableです。既存`score`は不変で、`slopeBurden`と`comparisonScore`を分離します。坂道希望が無効、または標高不明なら旧scoreの順位へ戻ります。
