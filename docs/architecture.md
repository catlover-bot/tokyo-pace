...
生成パイプラインは全15設定の順位と候補集約を`data/generated/field-candidate-ranking-sensitivity.json/.csv`、最終訪問候補を`field-visit-shortlist.json/.csv`へatomic writeします。`src/data/generated/`にはUIに必要な候補集約とshortlistだけを縮小同梱し、シナリオ全行は重複保存しません。field-check画面も基準単一スコア順位と訪問推奨順を別の配列として参照し、片方からもう片方を推測しません。
