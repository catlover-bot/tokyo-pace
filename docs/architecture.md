...
```text
公式possible候補 ─┐
                  ├→ 現地確認優先候補JSON/CSV → 読み取り専用field-check画面
既存の厳格地点 ──┘
...
現地確認候補抽出はデモルートごとの正規化進行距離を用い、候補を1地点追加した前後の最大空白を再計算します。同一施設は名称または住所の正規化一致と近接距離の両方を根拠にだけグループ化し、座標が近いだけの施設は統合しません。異名称・異住所の3地点以上が完全同一座標へ集中する場合は上流の座標品質異常として統合せず、現地確認順位から除外して件数をmetadataへ記録します。順位は改善量、影響ルート数、理論追加点との距離、ルートとの距離、候補IDによる辞書式比較で固定し、単一の代表指標がどのルート由来かを`primaryRouteId`で保持します。
