...
`data:update`は、現在の最長空白、理論上の休憩地点追加位置、ルートへの推定直線距離、複数デモルートへの有用性、公式施設区分を使い、同一施設の過大評価と既存厳格地点との重複を避けて上位10〜15地点（上限12）を`field-verification-candidates.json` / `.csv`へ固定順で生成します。異名称・異住所の多数レコードが同一座標へ集中する上流品質異常は、同一施設と決めつけて統合せず順位対象から除外します。複数ルート候補の単一指標には`primaryRouteId`を併記します。これは現地確認の優先順位であり、その施設で休憩できることや設備を設置できることの保証ではありません。

公開画面の`/?mode=field-check`は読み取り専用です。候補順位、住所、デモルートからの推定直線距離、選定理由、確認項目、理論上の改善、公式出典、CSV用IDを地図と一覧で確認でき、入力用CSVテンプレートをダウンロードできます。匿名の書込みAPI、写真アップロード、サーバー保存機能はありません。
