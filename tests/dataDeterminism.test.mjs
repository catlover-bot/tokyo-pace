...
describe("生成物の再現性", () => {
  ...
  it("公式レコード本体にretrievedAtを重複保存しない", async () => { const generated = JSON.parse(await readFile(new URL("../data/generated/official-toilets.json", import.meta.url), "utf8")); expect(generated.records).toHaveLength(9247); expect(generated.records.every((item) => !("retrievedAt" in item.source))).toBe(true); }, 30_000);
  it("全件版とブラウザ縮小版を区別する", async () => { const full = JSON.parse(await readFile(new URL("../data/generated/rest-candidates.json", import.meta.url), "utf8")); const browser = JSON.parse(await readFile(new URL("../src/data/generated/rest-candidates.json", import.meta.url), "utf8")); expect(full.candidates).toHaveLength(1907); expect(browser.candidates.length).toBeLessThan(full.candidates.length); expect(browser.records).toBeUndefined(); }, 30_000);
});
