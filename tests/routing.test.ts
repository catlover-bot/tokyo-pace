...
const strictCandidate: RestCandidate = { id: "rest", name: "確認済み休憩候補", latitude: 35.69, longitude: 139.695, address: null, category: "verified_rest_spot", confidence: "confirmed", openingHours: null, indoor: true, seating: true, drinkingWaterAvailable: null, wheelchairAccessible: null, source };
...
describe("動的経路の正規化と評価", () => {
  ...
  it("厳格候補の射影位置でwalkingSegmentsを分割する", () => { const segments = deriveDynamicWalkingSegments(normalized, [strictCandidate]); expect(segments).toHaveLength(2); expect(segments.reduce((sum, item) => sum + item.distanceMeters, 0)).toBeCloseTo(1000, 6); });
