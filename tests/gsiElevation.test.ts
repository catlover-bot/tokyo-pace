...
describe("GSI elevation enrichment", () => {
  ...
  it("keeps route usable and metrics unknown when GSI fails", async () => { const result = await enrichRoutesWithGsiElevation([route], vi.fn(async () => new Response("", { status: 503 })) as typeof fetch); expect(result[0].coordinates).toEqual(route.coordinates); expect(result[0].elevation?.status).toBe("unavailable"); expect(result[0].elevation?.totalAscentMeters).toBeNull(); expect(result[0].warnings?.join("")).toContain("坂道情報"); });
});
