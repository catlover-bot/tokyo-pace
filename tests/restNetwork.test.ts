...
describe("公式休憩候補データ", () => {
  ...
  it("公式・推定・現地確認済みと信頼度を区別する", () => { expect(deriveRestConfidence(candidate("v", 0, { category: "verified_rest_spot", source: { ...source, fieldVerifiedAt: "2026-01-01" } }))).toBe("confirmed"); expect(deriveRestConfidence(candidate("e", 0, { category: "estimated_rest_spot" }))).toBe("estimated"); expect(deriveRestConfidence(candidate("p", 0, { category: "public_facility", seating: null }))).toBe("possible"); });
});
